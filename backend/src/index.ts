...
import { extract, ground, verify } from "./executor.js";
...
const executorRequest = z.object({ step: z.object({ intent: z.string(), action: z.string(), target_description: z.string(), target_hints: z.record(z.string(), z.string().optional()), expected_page: z.string(), extract: z.object({ what: z.string(), format: z.string() }).optional() }), nodes: z.array(z.object({ role: z.string(), accessible_name: z.string(), reference_id: z.string(), nearby_text: z.string().optional() })).max(300), retryReason: z.string().optional(), screenshot: z.string().max(3_000_000).optional() });
app.post("/execute/ground", requireAuth, async (req, res, next) => { try { const body = executorRequest.parse(req.body); res.json(await ground(body.step, body.nodes, body.retryReason, body.screenshot)); } catch (error) { next(error); } });
app.post("/execute/verify", requireAuth, async (req, res, next) => { try { const body = executorRequest.parse(req.body); res.json(await verify(body.step, body.nodes)); } catch (error) { next(error); } });
app.post("/execute/extract", requireAuth, async (req, res, next) => { try { const body = executorRequest.parse(req.body); if (!body.step.extract) { res.status(400).json({ error: "An extract step is required." }); return; } res.json(await extract(body.step, body.nodes, body.screenshot)); } catch (error) { next(error); } });
