...
function ExecutionPanel({ execution, onPoint, onStop }: { execution: ExecutionState; onPoint: () => void; onStop: () => void }) {
  ...
  return <main className="min-h-screen space-y-4 p-4">
    <header><h1 className="text-lg font-semibold">Running procedure</h1><p className="text-xs text-slate-400">Self-healing execution is active.</p></header>
    ...
    {execution.paused && <section className="rounded-lg border border-amber-400 bg-amber-400/10 p-3"><h2 className="font-medium">Point at the right element</h2><p className="mt-1 text-sm">Looking for: {execution.paused.intent}</p><p className="mt-1 text-xs text-slate-300">Found: {execution.paused.found}. {execution.paused.reason}</p><button onClick={onPoint} className="mt-3 w-full rounded bg-amber-400 p-2 font-medium text-slate-950">Point at the right element</button></section>}
...
export function App() {
  ...
  if (review) return <main className="min-h-screen p-4"><Review procedure={review.procedure} rawCount={review.rawCount} onBack={() => setReview(null)} onRun={(procedure, inputValues) => void send({ type: "RUN_PROCEDURE", procedure, inputValues }).then(refresh).catch(report)} onSave={async (procedure) => { try { await send({ type: "SAVE_PROCEDURE", procedure }); setReview(null); await refresh(); } catch (e) { report(e); } }} /></main>;
